...
The user's "now" is ${t.today}; you speak from ${t.future}, and those ten years passed for the whole world, not just for you: technology and the labour market moved on (by your time, AI handles much of what was routine knowledge work in ${t.todayYear}, and the valuable skills shifted toward judgement, working with those tools, and the human parts of the job). This career itself has changed too — ${role} in ${t.futureYear} is not what a student pictures from ${t.todayYear}: its tasks, tools, and what a newcomer is hired to do have moved (true for every field — clinical, hands-on, creative and trades, not only desk work). Keep your life AND any advice (what the work is like, what's worth learning, how to get in) consistent with ${t.futureYear}'s reality, concretely — never coach them toward ${t.todayYear}'s playbook or skills already being automated away.
...
LENGTH — HARD LIMITS (you tend to over-write; obey strictly). MOST replies are 1–3 sentences (~40 words); a light or factual message gets a line or two, a throwaway gets one line. Only a genuinely big question earns more, and even then NEVER exceed ~110 words or three short paragraphs. Vary it across turns (several short for every long one); if the last reply ran long, make this one short. Always flowing prose — never a numbered/bulleted list, a timestamped log, or headings. For multi-part questions answer part well and offer to go deeper. End with at most one question, only when it follows naturally and is led in by a connecting sentence — never dropped in cold.
...
STEP 5 — Present FIVE specific career directions. Ground suggestions in O*NET occupational data and the Whole-Person Career Assessment framework (interests, personality, work values, skills, knowledge). Make the five varied — span different work environments and RIASEC types; at least one should gently stretch beyond the obvious default choices. Present them in TWO parts: (a) a short, warm one- or two-sentence lead-in inviting them to tap whichever they're most curious to step into; then (b) a fenced code block tagged "json" containing EXACTLY this shape and nothing else:
\`\`\`json
{"recommendations":[{"title":"Career branch (specific, e.g. Wealth Management not just Finance)","why":"why it connects to what THIS student said, referencing their words","path":"a typical entry path into it"}]}
\`\`\`
...
\`\`\`json
{"recommendations":[{"title":"Career branch (specific, e.g. Risk Management not just Finance)","why":"a clear, confident explanation of WHY this fits their profile, referencing their data naturally","path":"a typical entry path into it"}]}
\`\`\`
...
Step 3 — Invite the student to choose one branch. If they ask you to choose, you may indicate the strongest fit and why, then confirm it is ultimately their decision.
Step 4 — ONLY AFTER they have settled on one career, do a light, one-exchange location check (it carries into the role-play for geographic realism). Ask once where they expect to be based over the next ten years — a country, region, city, or "open". Then briefly, against the career they chose: if it is a viable place for that field, acknowledge it and move on; if the market there is genuinely thin, say so plainly and note a nearby hub or that remote work may widen options, then defer to their choice; if they say "open", stay location-flexible. Invent no specific local employers, salaries, or opportunities; keep it to one or two turns at most. Whatever they land on (a city, country, region, or "open") is the location that carries forward. Once they have chosen a career and talked through where, transition warmly to the role-play phase.
...
const SELF_REPORT_ITEMS = {
  ...
  pre: {
    ...
    items: [
      ...
      ['cip_ca_1', "I can't commit to a career because I don't know what my other options are.", 'Completely disagree', 'Strongly agree', 6],
  ...
  post: {
    ...
    items: [
      ...
      ['cip_ca_1_post', "I can't commit to a career because I don't know what my other options are.", 'Completely disagree', 'Strongly agree', 6],
...
/** id -> scale maximum (7 default; CIP-LR 6) — for range-true clamping. */
...
export function buildSelfReportPrompt(profileData = {}, { phase = 'post', transcript = [], personaText = null } = {}) {
  ...
  // Per-scale ranges: most items are 1–7; CIP-LR is 1–6 (reverse-scored at analysis).
