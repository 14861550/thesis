...
function normRec(r) { return REC_VALUES.has(r) ? r : 'direct'; }
...
export async function createSession({ condition, rec, study, pid, recruiter } = {}) {
  const cond = condition ? normCondition(condition) : await assignBalancedCondition();
  const { rows } = await query(
    `INSERT INTO sessions (condition, rec, study, pid, recruiter, status)
     VALUES ($1, $2, $3, $4, $5, 'started') RETURNING *`,
    [cond, normRec(rec), clip(study) || 'kangzhi', clip(pid), clip(recruiter)]
  );
  return rows[0];
}
...
export async function saveSession(id, partial = {}) {
  ...
  if (partial.recruiter !== undefined) { sets.push(`recruiter = $${i++}`); vals.push(clip(partial.recruiter)); }
...
export async function listSessions({ condition, status, recruiter, limit = 200 } = {}) {
  const { rows } = await query(
    `SELECT id, condition, rec, study, pid, recruiter, status, version, created_at, completed_at,
     ...
     WHERE ($1::text IS NULL OR condition = $1)
       ...
       AND ($4::text IS NULL OR recruiter = $4)
     ORDER BY created_at DESC
     LIMIT $3`,
    [condition || null, status || null, limit, recruiter || null]
  );
  return rows;
}
 ...
 *  - removes the participant's self-entered name (the one direct identifier in
 *    the profile) — a file labelled "de-identified" must never carry real names
 *  - strips the post-survey contact/email + interview-yes contact
 *  - redacts email addresses anywhere in free text / transcripts
 *  - places / other quasi-identifiers in free text still warrant manual review
 */
export function deidentifyStudy(study) {
  const s = JSON.parse(JSON.stringify(study));
  // Direct identifier — drop the self-entered name. (anonymizeStudy re-sets it
  // to an anonymous label afterwards for the /results surface.)
  if (s.profile) delete s.profile.name;
  if (s.postSurvey) {
    delete s.postSurvey.contact;
    delete s.postSurvey.email;
  }
  const scrub = (t) => (typeof t === 'string' ? t.replace(EMAIL_RE, '[EMAIL]') : t);
  const scrubTurns = (t) => { if (Array.isArray(t)) t.forEach((m) => { if (m && m.text) m.text = scrub(m.text); }); };
  // open-ended free text
  for (const k of ['oe_real', 'oe_broke', 'oe_voice', 'oe_shift']) {
    if (s.postSurvey && s.postSurvey[k]) s.postSurvey[k] = scrub(s.postSurvey[k]);
  }
  // transcripts — the main study phases…
  for (const phase of ['phaseB', 'phaseC']) {
    scrubTurns(s[phase] && s[phase].transcript);
  }
  // …and the post-study free continuation + career explorations. These are
  // recorded too (outside the main analysis) and surface in de-identified
  // exports and the /results page, so they can equally carry an email a
  // participant typed mid-chat — scrub them with the same rule.
  if (s.freeContinuation) {
    scrubTurns(s.freeContinuation.transcript);
    if (Array.isArray(s.freeContinuation.explorations)) {
      s.freeContinuation.explorations.forEach((e) => {
        if (!e) return;
        scrubTurns(e.transcript);
        scrubTurns(e.phaseBTranscript);
      });
    }
  }
  s.meta = s.meta || {};
