...
export function mountAdminRoutes(app) {
  ...
  app.get('/api/admin/sessions', requireAdmin, requireDb, async (req, res) => {
    try {
      ...
      console.error('list sessions failed:', err?.message || err);
  ...
  app.post('/api/admin/sessions', requireAdmin, requireDb, async (req, res) => {
    try {
      const row = await createSession({ condition: req.body && req.body.condition });
      const link = `/?session=${row.id}&condition=${row.condition}`;
      res.json({ id: row.id, condition: row.condition, link });
    } catch (err) {
  ...
  app.get('/api/admin/sessions/:id', requireAdmin, requireDb, async (req, res) => {
    try {
      ...
      console.error('get session failed:', err?.message || err);
