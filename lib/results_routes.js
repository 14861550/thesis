...
// Closeness is the single IOS item; the FSCS continuity pair was dropped on
// supervisor feedback (14 Jun 2026), so continuity is no longer reported.
...
function deltas(study) {
  ...
  const d = {};
  for (const k of ['vividness', 'closeness']) {
...
export function mountResultsRoutes(app) {
  ...
  app.get('/api/results/overview', requireResults, requireDb, async (req, res) => {
    try {
      ...
      const dAcc = { vividness: [], closeness: [] };
      const postAcc = { vividness: [], closeness: [], manip_checks: [] };
      for (const s of studies) {
        ...
        for (const k of ['vividness', 'closeness']) if (delta[k] != null) dAcc[k].push(delta[k]);
