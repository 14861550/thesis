 ...
 *
 * Gated by ADMIN_TOKEN (the single shared secret — the separate results token
 * was removed). Every payload is de-identified and name-stripped: no
 * contact/email, names replaced with anonymous labels (P01, P02…). These routes
 * never delete, never export PII, never launch runs — they only read. The
 * surface stays read-only even though it reuses the admin secret. If ADMIN_TOKEN
 * is unset, the surface is disabled (503).
 */
...
// The results surface is gated by ADMIN_TOKEN (the separate results token was
// removed — the admin token is the single shared secret). The view itself stays
// read-only and de-identified; the gate just reuses ADMIN_TOKEN.
const resultsConfigured = () => Boolean(process.env.ADMIN_TOKEN);
// Constant-time compare against ADMIN_TOKEN (the single shared secret) — same
// timing-side-channel hardening as the admin gate. False (never throws) on
// empty / length-mismatched input.
const validToken = (t) => {
...
function tokenFromReq(req) {
  ...
  // Accept either the /results cookie or an existing /admin session cookie, so a
  // logged-in admin can open /results without re-entering the token.
  if (cookies.results_token) return cookies.results_token;
...
async function requireResults(req, res, next) {
  try {
    if (!(await resultsConfigured())) return res.status(503).json({ error: 'Results disabled: set ADMIN_TOKEN.' });
...
export function mountResultsRoutes(app) {
  ...
  app.get(['/results', '/results/'], async (req, res) => {
    if (!(await resultsConfigured())) return res.status(503).send('Results disabled — set ADMIN_TOKEN in the server env.');
  ...
  // The results surface's React code, precompiled (npm run build) and served
  // GATED — same posture as index.html, so it never reaches an unauthenticated
  // client. The authenticated page requests it with the results/admin cookie.
  app.get('/results/results.js', requireResults, (req, res) => {
    res.type('application/javascript').sendFile(path.join(RESULTS_DIR, 'results.js'));
  });
