...
// The distal outcome is CIP-Short Lack of Readiness (cip_lr_1..5, 1–6), ALL
// reverse-scored: per-item score = 7 − raw, then averaged.
const lrMean = (xs) => {
  const v = xs.filter((x) => typeof x === 'number' && !Number.isNaN(x)).map((x) => 7 - x);
  return v.length ? v.reduce((a, b) => a + b, 0) / v.length : null;
};
const OUTCOME_KEYS = ['vividness', 'closeness', 'continuity', 'cip_lr'];
function outcomesPre(study) {
  ...
  return {
    ...
    cip_lr: lrMean([p.cip_lr_1, p.cip_lr_2, p.cip_lr_3, p.cip_lr_4, p.cip_lr_5]),
  };
}
function outcomesPost(study) {
  ...
  return {
    ...
    cip_lr: lrMean([p.cip_lr_1_post, p.cip_lr_2_post, p.cip_lr_3_post, p.cip_lr_4_post, p.cip_lr_5_post]),
...
export function mountResultsRoutes(app) {
  ...
  app.get('/api/results/overview', requireResults, requireDb, async (req, res) => {
    try {
      ...
      const dAcc = { vividness: [], closeness: [], continuity: [], cip_lr: [] };
      const postAcc = { vividness: [], closeness: [], continuity: [], cip_lr: [], manip_checks: [] };
